...
{
    public partial class ad_forgate : System.Web.UI.Page
    {
        ...
        {
            SqlCommand cmd = new SqlCommand("select ad_password from admin where ad_id = '" + txtforgate.Text + "'", con);
            ...
            if (dt.Rows.Count > 0)
            {
                string password = dt.Rows[0]["ad_password"].ToString();
