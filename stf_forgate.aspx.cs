...
{
    public partial class stf_forgate : System.Web.UI.Page
    {
        ...
        {
            SqlCommand cmd = new SqlCommand("select stf_password from staff where stf_email = '" + txtforgate.Text + "'", con);
            ...
            if (dt.Rows.Count > 0)
            {
                string password = dt.Rows[0]["stf_password"].ToString();
